...
export default function Navbar() {
    ...
    let [pillStyle, setPillStyle] = useState({ left: 0, width: 111 });
    ...
    const magicPill = (e) => {
        ...
        let finalWidth = isActive ? e.currentTarget.offsetWidth : e.currentTarget.offsetWidth + 24;
        let finalLeft = !isActive && parentChildren.indexOf(e.currentTarget) > parentChildren.indexOf(currentActiveTab)? e.currentTarget.offsetLeft-24 : e.currentTarget.offsetLeft
        if (e.target.id == 'home') {
            setPillStyle({ left: 6, width: 111 });
            return
        }
        setPillStyle({
            left: finalLeft,
    ...
    return (
        <>
            <div className="fixed top-0 left-0 flex w-full z-50 pointer-events-none px-6 lg:px-12 py-6 justify-between items-center">
                <div>
                    <div>
                        <NavLink onClick={magicPill} to={'/'} className={`pointer-events-auto`}>
                            <div>
                ...
                <nav className={`hidden lg:flex relative glass-header desktop-nav items-center rounded-full pointer-events-auto gap-1 ${NavbarStyles.nav}`}>
                    ...
                    <div className={`rounded-full absolute left-0 opacity-100 bg-white -z-10 ${NavbarStyles.pillIndicator} ${NavbarStyles.pillTransition}`} style={{ left: pillStyle.left, width: pillStyle.width }}>

                    </div>

                    <NavLink onClick={magicPill} className={`relative z-10 flex items-center justify-center gap-2 px-6 rounded-full text-sm font-medium whitespace-nowrap text-white/60 ${NavbarStyles.navItem}`} to="/" >
                        <span className={`text-black ${NavbarStyles.icon}`}>
                            <svg xmlns="http://www.w3.org/2000/svg ${NavbarStyles.icon}" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide-icon lucide lucide-house w-4 h-4"><path d="M15 21v-8a1 1 0 0 0-1-1h-4a1 1 0 0 0-1 1v8"></path><path d="M3 10a2 2 0 0 1 .709-1.528l7-6a2 2 0 0 1 2.582 0l7 6A2 2 0 0 1 21 10v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"></path></svg>
                        </span>
                        Home
                    </NavLink>
                    ...
                    <NavLink onClick={magicPill} className={`relative z-10 flex items-center justify-center gap-2 px-6 rounded-full text-sm font-medium whitespace-nowrap text-white/60 ${NavbarStyles.navItem}`} to="series" >
                        <span className={`text-black ${NavbarStyles.icon}`}>
                            <svg xmlns="http://www.w3.org/2000/svg ${NavbarStyles.icon}" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide-icon lucide lucide-tv w-4 h-4"><path d="m17 2-5 5-5-5"></path><rect width="20" height="15" x="2" y="7" rx="2"></rect></svg>
                        </span>
                        Shows
                    </NavLink>
                    ...
                    <NavLink onClick={magicPill} className={`relative z-10 flex items-center justify-center gap-2 px-6 rounded-full text-sm font-medium whitespace-nowrap text-white/60 ${NavbarStyles.navItem}`} to="lists" >
                        <span className={`text-black ${NavbarStyles.icon}`}>
                            <svg xmlns="http://www.w3.org/2000/svg ${NavbarStyles.icon}" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide-icon lucide lucide-bookmark w-4 h-4"><path d="m19 21-7-4-7 4V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2v16z"></path></svg>                        </span>
                        My List
                    </NavLink>

                    <span className={`w-px h-4.5 m-1.5 ${NavbarStyles.divider}`}></span>

                    <button onClick={magicPill} className={`relative z-10 flex items-center justify-center rounded text-white/60 ${NavbarStyles.searchIndicator}`}>
                        <NavLink to='search'>
                            <div>
